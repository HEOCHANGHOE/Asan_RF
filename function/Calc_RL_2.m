function [stat, Mat_RL, status, msg] = Calc_RL(img_data, mask, bins, type, alpha)
% By Bumwoo Park
% Update: 2018-01-23
...
[X_idx, Y_idx] = ind2sub(size(img_data), On_Mask);
...
for idx_dir = 1 : length(Dir_X)
   ...
   for idx_voxel = 1 : Ele_Mask
       ...
       if (Mat_visit(X_idx(idx_voxel), Y_idx(idx_voxel)) == 1)
          continue; 
       end
       
       % 5.2 Get current signal intensity           
       SI_cur = img_quantized(X_idx(idx_voxel), Y_idx(idx_voxel));
       cur_length = 1;
       for dir_sign = [1 -1]
           for dist = 1 : max_RL
               ...
               SI_next = img_quantized(Next_X, Next_Y);
               
               % 5.6 Compare current and next signal intensity
               if SI_cur == SI_next
                   cur_length = cur_length+1;
                   Mat_visit(Next_X, Next_Y) = 1;
               else
                   break;
               end
           end
